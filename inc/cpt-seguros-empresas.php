<?php
/**
 * Custom Post Type: Seguros Empresas
 * Theme: Pacto 25
 * 
 * Strict Agency SOP:
...
/**
 * Register Seguros Empresas Post Type
 */
function pacto_register_cpt_seguros_empresas() {
    $labels = array(
        'name'                  => _x( 'Seguros Empresas', 'Post type general name', 'pacto-25' ),
        'singular_name'         => _x( 'Seguro Empresa', 'Post type singular name', 'pacto-25' ),
        'menu_name'             => _x( 'Seguros Empresas', 'Admin Menu text', 'pacto-25' ),
        'name_admin_bar'        => _x( 'Seguro Empresa', 'Add New on Toolbar', 'pacto-25' ),
        'add_new'               => __( 'Adicionar Novo', 'pacto-25' ),
        'add_new_item'          => __( 'Adicionar Novo Seguro', 'pacto-25' ),
        'new_item'              => __( 'Novo Seguro Empresa', 'pacto-25' ),
        'edit_item'             => __( 'Editar Seguro Empresa', 'pacto-25' ),
        'view_item'             => __( 'Ver Seguro', 'pacto-25' ),
        'all_items'             => __( 'Todos os Seguros Empresas', 'pacto-25' ),
    ...
    $args = array(
        ...
        'show_in_menu'       => true,
        'query_var'          => true,
        'rewrite'            => array( 'slug' => 'seguros-empresas', 'with_front' => false ),
    ...
    register_post_type( 'seguro_empresa', $args );
}
add_action( 'init', 'pacto_register_cpt_seguros_empresas', 0 );

/**
 * Add Meta Box for Button Details if ACF is not active
 */
function pacto_seguro_empresa_meta_box() {
    add_meta_box(
        'pacto_seguro_empresa_details',
        __( 'Opções do Seguro', 'pacto-25' ),
        'pacto_seguro_empresa_meta_box_render',
        'seguro_empresa',
        'normal',
        'high'
    );
}
add_action( 'add_meta_boxes', 'pacto_seguro_empresa_meta_box' );

function pacto_seguro_empresa_meta_box_render( $post ) {
    wp_nonce_field( 'pacto_seguro_empresa_meta_save', 'pacto_seguro_empresa_nonce' );
...
/**
 * Save Seguro Empresa Meta
 */
function pacto_save_seguro_empresa_meta( $post_id ) {
    if ( ! isset( $_POST['pacto_seguro_empresa_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['pacto_seguro_empresa_nonce'] ), 'pacto_seguro_empresa_meta_save' ) ) {
        return;
    }
...
add_action( 'save_post_seguro_empresa', 'pacto_save_seguro_empresa_meta' );

/**
 * Custom Admin Columns for Seguros Empresas
 */
function pacto_seguro_empresa_columns( $columns ) {
    $new_columns = array();
...
add_filter( 'manage_seguro_empresa_posts_columns', 'pacto_seguro_empresa_columns' );

function pacto_seguro_empresa_custom_column( $column, $post_id ) {
    switch ( $column ) {
        case 'thumb':
...
add_action( 'manage_seguro_empresa_posts_custom_column', 'pacto_seguro_empresa_custom_column', 10, 2 );

/**
 * Make Order Column Sortable
 */
function pacto_seguro_empresa_sortable_columns( $columns ) {
    $columns['menu_order'] = 'menu_order';
    return $columns;
}
add_filter( 'manage_edit-seguro_empresa_sortable_columns', 'pacto_seguro_empresa_sortable_columns' );
